...
function GetTemperaturer(){
    $.get("/temperature",function(data, status){
        if (status == "success"){
            d = JSON.parse(data)
            $("#temperature").text(d.Temperature.toFixed(1) + " ℃");
        }else{
            alert("Data: " + data + "\nStatus: " + status);
        }
    });
...
function refreshRelay(){
	$.get("/relaystatus",function(d, status){
		if (status == "success"){
            data=JSON.parse(d)
			if (data.status == "on"){
				setRelayStyle( true );
